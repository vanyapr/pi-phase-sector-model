...
018 & Machin external certificate & Использовать независимую формулу Мэчина как алгебраически вычисляемый источник сертификата. & На искусственных границах сертификат, не использующий скрытый десятичный хвост, схлопнул $10^{10}$ кандидатов до одного блока. \\
\bottomrule
\end{longtable}

\section*{Главная экспериментальная граница}
Эксперименты 001--009 показывают, какие простые предикторы не дают результата.  Эксперименты 010--018 показывают положительный контур: конечный поиск и сертифицированное продолжение при наличии внешнего секторного сертификата.  В Experiment 018 внешний сертификат строится алгебраически из формулы Мэчина, что показывает: сертификат является вычисляемым объектом, а не предположением о знании будущих десятичных цифр.  Поэтому итоговая граница такова:
\begin{itemize}
...
Архив содержит пакеты \texttt{pi\_fs\_experiment\_001\_package.zip}--\texttt{pi\_fs\_experiment\_018\_package.zip}, таблицы, графики и исходный модуль \texttt{pifs/proportion.py}.  Основные CSV-файлы вынесены в каталог \texttt{tables/} основного пакета.
